...
export const createMaster = async (name, arr, rating_id) => {
	const { data } = await instance.post('/api/master/create', { name, arr, rating_id });
	return data;
}
...
export const updateMaster = async (masterId, newName, ratingId) => {
	const { data } = await instance.put('/api/master/update', { masterId, newName, ratingId });
	return data;
}