...
const TableMaster = ({
    ...
    const handleUpdate = (id, name, ratingId, cities) => {
        ...
        setUpdateChangeCity(
            cities.map((c) => ({ value: c.id, label: c.title })),
        )
        console.log(cities)
    ...
    const updateMaster = () => {
        ...
        if (
            ...
            ratingIdUpdate === oldRatingId &&
            arrCityId.join() === oldChangeCity
        ) {
            setModalActiveUpdade(false)
            return
        }
        if (oldName !== nameUpdate) {
            const dublicateName = master.find(
                (m) => m.master_name === nameUpdate.trim(),
            )
        ...
        <>
            <Modal active={modalActiveUpdade} setActive={setModalActiveUpdade}>
                <form onSubmit={handleSubmit}>
                    ...
                    <button
                        className="auth__btn"
                        disabled={!updateChangeCity.length}
                    >
                        Ok
                    </button>
                </form>
                    ...
                    {master.map((item) => (
                        <tr key={item.master_id}>
                            ...
                            <td>
                                <button
                                    className="auth__btn"
                                    onClick={() => removeMaster(item.master_id)}
                                >
                                    <svg width="24px" height="24px">
